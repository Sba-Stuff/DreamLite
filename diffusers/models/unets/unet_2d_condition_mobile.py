# Copyright (c) 2025 ByteDance Ltd. and its Intelligent Creation Lab
#
...
from ..transformers.transformer_hunyuan_video import HunyuanVideoTokenRefiner
...
class SnapGenUNet2DConditionModel(DreamLiteUNetModel):
    @register_to_config
    def __init__(
        self,
        *args,
        ...
        **kwargs
        ...
        # Rebuild mid_block with multiple layers if needed
        if self.num_mid_layers > 1:
            self.mid_block = nn.ModuleList([self._create_mid_block() for _ in range(num_mid_layers)])
        else:
            self.mid_block = self._create_mid_block()

        # Rebuild down_blocks and up_blocks with corrected channel indexing
        self.down_blocks, self.up_blocks = self._rebuild_blocks()

    def _create_mid_block(self):
        return get_mid_block(
            self.config.mid_block_type,
            temb_channels=self.blocks_time_embed_dim,
            ...
            attention_head_dim=self.config.attention_head_dim[-1],
    ...
    def _rebuild_blocks(self):
        ...
        for i, down_block_type in enumerate(self.config.down_block_types):
            ...
            down_block = get_down_block(
                ...
                out_channels=output_channel,
                temb_channels=self.blocks_time_embed_dim,
                add_downsample=not (i == len(self.config.down_block_types)-1),
                resnet_eps=self.config.norm_eps,
                resnet_act_fn=self.config.act_fn,
                resnet_groups=self.config.norm_num_groups,
                cross_attention_dim=self.config.cross_attention_dim[i],
                num_attention_heads=self.config.num_attention_heads[i+1],
                ...
                cross_attention_norm=self.config.cross_attention_norm,
                attention_head_dim=self.config.attention_head_dim[i+1],
                dropout=self.config.dropout,
                qk_norm=self.config.qk_norm,
                use_sep_conv=self.config.use_sep_conv,
                ff_mult=self.config.ff_mult,
        ...
        # Rebuild up_blocks with corrected channel indexing
        ...
        for i, up_block_type in enumerate(self.config.up_block_types):
            ...
            up_block = get_up_block(
                up_block_type,
                num_layers=self.config.layers_per_block[i]+1,
                transformer_layers_per_block=self.config.transformer_layers_per_block[i],
                in_channels=input_channel,
                out_channels=output_channel,
                prev_output_channel=input_channel,
                temb_channels=self.blocks_time_embed_dim,
                add_upsample=True,
                resnet_eps=self.config.norm_eps,
                resnet_act_fn=self.config.act_fn,
    ...
    def forward(
        ...
        if self.mid_block is not None:
            if self.num_mid_layers > 1:
                for i, mid_block in enumerate(self.mid_block): 
                    if hasattr(self.mid_block, "has_cross_attention") and self.mid_block.has_cross_attention:
                        sample = mid_block(
                            sample,
                            emb,
