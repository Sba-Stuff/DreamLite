...
public class Qwen35: Module, VLMModel {
    ...
    public func getHiddenStates(inputIds: MLXArray) -> MLXArray {

        // 1. Token Embedding
        var h = self.languageModel.model.embedTokens(inputIds)

        // 2. 构建 additive causal mask（下三角=0，上三角=-∞）
        let seqLen = inputIds.dim(1)
        let causalMask = Self.makeAdditiveCausalMask(length: seqLen)

        // 3. 逐层通过 Decoder Layers
        for layer in self.languageModel.model.layers {
            h = layer(h, attentionMask: causalMask, ssmMask: nil, cache: nil, positionIds: nil)
        }

        // 4. Final RMSNorm
        h = self.languageModel.model.norm(h)

        return h  // [1, seq_len, 1024]
    }

    // MARK: - 图文混合编码
    /// 提取最后一层 Transformer 的 hidden states（文本 + 图像输入）
    /// 图像 token 替换 input 中的 image placeholder，合并后统一过 Transformer。
    /// - Parameters:
    ///   - inputIds:     Token ID 数组，shape [1, seq_len]，包含 image placeholder tokens
    ///   - pixelValues:  预处理后的图像张量
    ///   - imageGridTHW: 视觉模型所需的 Temporal/Height/Width 网格尺寸
    /// - Returns: Hidden states，shape [1, total_seq_len, 1024]
    public func getHiddenStatesWithImage(
        inputIds: MLXArray,
        ...
        let (imageFeatures, _) = self.visionModel(pixelValues, gridTHW: imageGridTHW)
        // imageFeatures shape: [num_image_tokens, hidden_size]  (e.g. [64, 1024])

        // ---- 2. Language Embeddings ----
        var h = self.languageModel.model.embedTokens(inputIds)
        // h shape: [1, seq_len, hidden_size]

        let seqLen     = h.dim(1)
        let hiddenSize = h.dim(2)

        // ---- 3. 将视觉 token 合并到文本 embedding 序列中 ----
        // image placeholder token ID（Qwen 系列 <|image_pad|>）
        // 如果你的 config 里有 imageTokenId，可改为 config.imageTokenId
        let imageTokenId: Int32 = 151655

        let flatInputIds = inputIds.reshaped(-1)                    // [seq_len]
        let imageMask    = flatInputIds .== MLXArray(imageTokenId)  // [seq_len] Bool

        // cumsum 技巧：为每个 image 位置生成递增下标，映射到 imageFeatures
        let maskInt     = imageMask.asType(.int32)
        let cumIndices  = MLX.cumsum(maskInt, axis: 0) - 1          // 0-based
        let safeIndices = MLX.clip(cumIndices,
                                   min: MLXArray(Int32(0)),
                                   max: MLXArray(Int32(imageFeatures.dim(0) - 1)))

        let flatFeats     = imageFeatures.reshaped(-1, hiddenSize)  // [num_img, hidden]
        let expandedFeats = flatFeats[safeIndices]                  // [seq_len, hidden]

        // 按 mask 选择：image 位置 → vision 特征，其余 → 文本 embedding
        var flatH  = h.reshaped(seqLen, hiddenSize)
        let mask2D = imageMask.reshaped(seqLen, 1)
        flatH = MLX.where(mask2D, expandedFeats, flatH)
        h = flatH.reshaped(1, seqLen, hiddenSize)

        // ---- 4. Transformer Forward ----
        let causalMask = Self.makeAdditiveCausalMask(length: seqLen).asType(.bfloat16)
    ...
    /// additive causal mask：下三角(含对角)=0, 上三角=-∞
    /// 返回 shape [1, 1, length, length]，可广播到 [batch, heads, seq, seq]
    private static func makeAdditiveCausalMask(length: Int) -> MLXArray {
        let rows = MLXArray(0 ..< Int32(length)).reshaped(length, 1)
        let cols = MLXArray(0 ..< Int32(length)).reshaped(1, length)
        let lower = rows .>= cols
        let mask = MLX.where(lower,
                         MLXArray(Float(0)),
    ...
    func cumulativeSplitIndices(from sizes: [Int]) -> [Int] {
        var sum = 0
